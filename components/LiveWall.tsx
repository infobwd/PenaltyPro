...
import { isGoalkeeper, playerPositionLabel, playerPositionRank } from '../services/playerPositions';
...
/**
 * ลิงก์วิดีโอ -> URL สำหรับฝัง iframe
 *
 * ⚠️ ส่วน YouTube ต้องใช้ youTubeId/youTubeEmbed ตัวเดียวกับหน้า /live
 *
 * ของเดิมแยกเขียนเองที่นี่ และรู้จักแค่ `?v=` กับ `youtu.be/` — ลิงก์ไลฟ์จริง
 * ที่เจ้าภาพก๊อปมาคือ `youtube.com/live/xxxxxxxxxxx` ซึ่งแปลงไม่ได้ แล้ว
 * คืน null ทำให้ iframe ได้ src="" = จอดำ ทั้งที่หน้า /live เล่นคลิปเดียวกันได้ปกติ
 * (รวมถึง /embed/ กับ /shorts/ ที่ของเดิมก็อ่านไม่ออกเหมือนกัน)
 */
/**
 * วันและเวลาของนัด แยกกันเพื่อวางคนละบรรทัดบนจอ
 *
 * คืนวันเป็นค่าว่างเมื่อเป็นวันนี้ — จอในสนามขึ้นคู่ของวันนี้เป็นหลัก
 * ถ้าพิมพ์ "วันนี้" ทุกใบจะเปลืองพื้นที่โดยไม่ได้บอกอะไรเพิ่ม
 */
const matchWhen = (m: Match): { date: string; time: string } => {
    const raw = m.scheduledTime || m.date;
    const when = new Date(raw);
    if (!raw || Number.isNaN(when.getTime())) return { date: '', time: '-' };
    const today = new Date();
    const sameDay = when.getFullYear() === today.getFullYear()
        && when.getMonth() === today.getMonth() && when.getDate() === today.getDate();
    return {
        date: sameDay ? '' : when.toLocaleDateString('th-TH', { day: 'numeric', month: 'short' }),
        time: when.toLocaleTimeString('th-TH', { hour: '2-digit', minute: '2-digit' }),
    };
};
...
const getEmbedUrl = (url: string, autoplay: boolean = true, muted: boolean = true) => {
    ...
    if (youTubeId(url) !== '') {
        // จอใหญ่ต้องวนคลิปเองเมื่อจบ ไม่งั้นค้างที่เฟรมสุดท้ายจนกว่าสไลด์จะเปลี่ยน
        return youTubeEmbed(url, { autoplay, muted, loop: true });
    }
    if (url.includes('facebook.com')) {
        const encodedUrl = encodeURIComponent(url);
        return `https://www.facebook.com/plugins/video.php?href=${encodedUrl}&show_text=false&t=0&autoplay=${autoplay ? 1 : 0}&mute=${muted ? 1 : 0}&controls=1`;
    }
    return null;
};
...
/**
 * ลำดับสไลด์ — ย้ายออกมานอกคอมโพเนนต์เพื่อให้ตัวอ่าน URL ใช้แปลงชื่อเป็นเลขได้
 * ลำดับนี้คือสัญญากับผู้ตั้งค่า OBS แล้ว (`?slide=9` = Sponsors) การสลับตำแหน่ง
 * จะทำให้ฉากที่ตั้งไว้เปลี่ยนเนื้อหาโดยไม่มีใครรู้ — เพิ่มต่อท้ายได้ แต่อย่าสลับ
 */
const SLIDE_NAMES = ['Matches', 'MatchForecast', 'Standings', 'Bracket', 'Results',
  'TopScorers', 'TopKeepers', 'FanPrediction', 'Highlights', 'Sponsors',
  'Versus', 'LiveStream', 'MatchHighlights', 'Lineup'];
...
const LiveWall: React.FC<LiveWallProps> = ({ matches, teams, players, config, predictions, onClose, onRefresh, tournamentId = 'default', currentUser }) => {
  /*
   * โหมดใช้เป็นฉากใน OBS
   *
   * หน้านี้ถูกออกแบบมาให้คนกดดูเอง แต่ระบบถ่ายทอดสดต้องเอาไปเป็น Browser Source
   * ในฉาก "เปิดรายการ / ผลการแข่งขัน / ผู้สนับสนุน / พักเบรก" ซึ่งไม่มีใครกดอะไรได้
   * จึงต้องสั่งผ่าน URL ได้สามอย่าง — ทั้งหมดเป็นการอ่านเพิ่ม ไม่แตะพฤติกรรมเดิม
   *
   *   ?pin=1234        ปลดล็อกเอง (ไม่มีใครพิมพ์ PIN ให้ Browser Source ได้)
   *   ?slide=9         ล็อกไว้ที่สไลด์เดียว ไม่ต้องวนอัตโนมัติ
   *                    (ใส่ชื่อก็ได้ เช่น ?slide=Sponsors)
   *   ?kiosk=1         ซ่อนปุ่มตั้งค่า/ปิด/เต็มจอ — ปุ่มพวกนี้ไม่มีประโยชน์บนจอถ่ายทอด
   *                    และถ้าเผลอคลิกโดนกลางออกอากาศคือหน้าจอเปลี่ยนต่อหน้าคนดู
   *
   * ⚠️ PIN ที่ใส่ใน URL จะไปอยู่ในไฟล์ตั้งค่าของ OBS บนเครื่องถ่ายทอด
   *    ยอมรับได้เพราะเป็นเครื่องในสนามที่ผู้ดูแลคุมเอง แต่ไม่ควรเอา URL นี้
   *    ไปแปะที่อื่น
   */
  const wallParams = useMemo(() => {
    if (typeof window === 'undefined') return { pin: '', slide: null as number | null, kiosk: false };
    const q = new URLSearchParams(window.location.search);
    const raw = (q.get('slide') || '').trim();
    let slide: number | null = null;
    if (raw !== '') {
      const byNumber = Number(raw);
      // รับได้ทั้งเลขและชื่อ เพราะคนตั้งค่า OBS จำ "Sponsors" ง่ายกว่าจำเลข 9
      slide = Number.isFinite(byNumber)
        ? byNumber
        : SLIDE_NAMES.findIndex(s => s.toLowerCase() === raw.toLowerCase());
      if (slide === null || slide < 0 || slide >= SLIDE_NAMES.length) slide = null;
    }
    return { pin: q.get('pin') || '', slide, kiosk: q.get('kiosk') === '1' };
  }, []);

  // ... (Existing state and effects)
  const [isAuthenticated, setIsAuthenticated] = useState(
    currentUser?.role === 'admin' || wallParams.pin.trim() === (config.adminPin || '1234'));
  const [pinInput, setPinInput] = useState('');
  const [pinError, setPinError] = useState(false);
  const [currentSlide, setCurrentSlide] = useState(wallParams.slide ?? 0);
  ...
  /*
   * ด่าน "Tap to Start" มีไว้เพื่อให้เบราว์เซอร์ยอมเล่นเสียง (ต้องมีคนแตะก่อน)
   * แต่ Browser Source ใน OBS ไม่มีใครแตะได้ ฉากจะค้างอยู่ที่หน้านี้ตลอดกาล
   * โหมด kiosk จึงข้ามไปเลย และไม่เล่นเสียงด้วย ซึ่งถูกต้องอยู่แล้วเพราะ
   * เสียงของงานถ่ายทอดมาจากมิกเซอร์ ไม่ใช่จากหน้าเว็บ
   */
  const [hasInteracted, setHasInteracted] = useState(wallParams.kiosk);
  ...
  /**
   * ข้อความบนแถบวิ่งด้านล่าง = ประกาศของผู้จัด + คำขอบคุณผู้สนับสนุน
   *
   * ผู้สนับสนุนได้พื้นที่สองแบบบนจอ: โลโก้ (สไลด์ Sponsors กับมุมขวาล่าง) และ
   * ข้อความวิ่งตรงนี้ — เพราะรายที่สนับสนุนเป็นสิ่งของหรือรายที่ยังไม่ได้ส่งโลโก้มา
   * จะไม่มีที่ยืนบนจอเลยถ้ามีแต่ช่องโลโก้ ทั้งที่ควรได้รับการขอบคุณเท่ากัน
   */
  const announcements = useMemo(() => {
    const activeTickers = tickerMessages.filter(t => t.isActive
      && (!t.type || !t.type.includes('::') || t.type.includes(`::${tournamentId}`)));
    const base = activeTickers.length > 0
      ? activeTickers.map(t => t.message)
      : (config.announcement ? config.announcement.split('|').filter(s => s.trim() !== '') : []);

    const thanks = sponsors.map(s => {
      const amount = Number(s.contributionAmount || 0);
      const detail = (s.contributionDetail || '').trim();
      if (s.contributionType === 'Money' && amount > 0) {
        return `ขอขอบคุณ ${s.name} สนับสนุน ${amount.toLocaleString('th-TH')} บาท`;
      }
      if (s.contributionType === 'Goods') {
        return `ขอขอบคุณ ${s.name} สนับสนุน${detail !== '' ? ` ${detail}` : 'สิ่งของ'}`;
      }
      return `ขอขอบคุณ ${s.name} ร่วมสนับสนุนการแข่งขัน`;
    });

    return [...base, ...thanks];
  }, [tickerMessages, config.announcement, tournamentId, sponsors]);
  const currentUrl = window.location.href.split('?')[0];

  const loadExtras = async () => { try { const [contestData, sponsorData, musicData, tickerData] = await Promise.all([fetchContests(), fetchSponsors(), fetchMusicTracks(), fetchTickerMessages()]); /* ภาพถ่ายต้องเป็นของรายการที่กำลังจัดเท่านั้น
       เดิมดึงจากทุกการประกวดที่เคยมี จอในสนามระหว่างครั้งที่ 5 จึงขึ้นภาพครั้งที่ 3 ปนมา
       การประกวดที่ไม่ผูกรายการ (tournamentId ว่าง) ถือเป็นของส่วนกลาง ใช้ได้ทุกงาน */
    const scopedContestIds = new Set(contestData.contests
      .filter(c => !c.tournamentId || c.tournamentId === tournamentId)
      .map(c => c.id));
    const scopedEntries = contestData.entries.filter(e => scopedContestIds.has(e.contestId));
    const uniquePhotos = scopedEntries.filter((e, i, a) => a.findIndex(t => t.photoUrl === e.photoUrl) === i); setContestEntries(uniquePhotos); const filterForDisplay = (itemType: string | undefined) => { if (!itemType || !itemType.includes('::')) return true; return itemType.includes(`::${tournamentId}`); }; setSponsors(sponsorData.filter(s => filterForDisplay(s.type))); setMusicTracks(musicData.filter(m => filterForDisplay(m.type))); setTickerMessages(tickerData); } catch (e) { console.error("Failed to load live wall extras"); } };
  ...
  const handleTouchEnd = () => { if (wallParams.slide !== null) return; if (!touchStart || !touchEnd) return; const distance = touchStart - touchEnd; if (distance > 50) setCurrentSlide(prev => (prev + 1) % totalSlides); if (distance < -50) setCurrentSlide(prev => (prev - 1 + totalSlides) % totalSlides); setTouchStart(null); setTouchEnd(null); };
  ...
  // LIVE STREAM LOGIC
  /**
   * คู่ที่กำลังถ่ายทอดสด — เกณฑ์เดียวกับหน้า /live
   *
   * ตรวจว่าลิงก์อ่านเป็นรหัสวิดีโอได้จริง ไม่ใช่แค่ "ช่องไม่ว่าง" — ค่าที่พิมพ์ผิด
   * หรือเหลือแต่เว้นวรรคจะทำให้สไลด์ขึ้นจอดำโดยไม่มีอะไรบอกว่าเพราะอะไร
   */
  ...
  const lineupTeams = useMemo(() => teams
    ...
    .map(t => ({
      team: t,
      list: players.filter(p => p.teamId === t.id)
        .sort((a, b) => playerPositionRank(a.position) - playerPositionRank(b.position)
  ...
  const finishedStreamingMatches = useMemo(() => matches
    ...
    .map(m => youTubeId(m.highlightUrl) !== ''
      ? { ...m, livestreamUrl: m.highlightUrl, _isHighlight: true } as Match & { _isHighlight?: boolean }
      : m)
    /**
     * มีคลิปไฮไลต์ = เอาขึ้นจอได้เลย ไม่ต้องรอให้นัดนั้นมีผู้ชนะ
     *
     * เดิมบังคับ m.winner ทั้งสองทาง ทำให้คลิปไฮไลต์ที่เจ้าภาพใส่ไว้ในนัดที่
     * ยังไม่ได้บันทึกผล (status ยังเป็น Scheduled) ไม่ขึ้นเลยสักคลิป —
     * ซึ่งเป็นสภาพจริงบนหน้างาน เพราะคนตัดคลิปกับคนจดผลเป็นคนละคนกัน
     *
     * ส่วนลิงก์ถ่ายทอดสดยังต้องมีผู้ชนะเหมือนเดิม ไม่งั้นคู่ที่กำลังแข่งอยู่
     * จะไปโผล่ในสไลด์ "ดูย้อนหลัง" พร้อมกับสไลด์ถ่ายทอดสด
     */
    .filter(m => youTubeId(m.livestreamUrl) !== ''
      && ((m as Match & { _isHighlight?: boolean })._isHighlight || m.winner))
    .sort((a, b) => new Date(b.date || b.scheduledTime || 0).getTime()
      - new Date(a.date || a.scheduledTime || 0).getTime()), [matches]);
  ...
  const topScorers = useMemo(() => { const scores: Record<string, {name: string, team: string, goals: number, photoUrl?: string}> = {}; matches.forEach(m => { const processGoal = (player: string, teamId: string) => { const pName = String(player || '').split('(')[0].replace(/[#0-9]/g,'').trim(); if (!pName) return; const teamName = teamId === 'A' || (typeof m.teamA==='string'?m.teamA:m.teamA.name) === teamId ? (typeof m.teamA==='string'?m.teamA:m.teamA.name) : (typeof m.teamB==='string'?m.teamB:m.teamB.name); const key = `${pName}_${teamName}`; if (!scores[key]) { const teamObj = teams.find(t => t.name === teamName); const playerObj = players.find(p => p.teamId === teamObj?.id && p.name.includes(pName)); scores[key] = { name: pName, team: teamName, goals: 0, photoUrl: playerObj?.photoUrl }; } scores[key].goals++; }; m.events?.forEach(e => { if (e.type === 'GOAL') processGoal(e.player, e.teamId); }); m.kicks?.forEach(k => { if (k.result === KickResult.GOAL) processGoal(k.player, k.teamId); }); }); return Object.values(scores).sort((a,b) => b.goals - a.goals).slice(0, 5); }, [matches, teams, players]);
  const topKeepers = useMemo(() => {
    const savesMap: Record<string, {
      teamName: string; keeperName?: string; photoUrl?: string;
      saves: number; cleanSheets: number; logoUrl?: string;
    }> = {};
    teams.forEach(t => {
      if (t.status !== 'Approved') return;
      const keepers = players.filter(p => p.teamId === t.id && isGoalkeeper(p.position));
      savesMap[t.name] = {
        teamName: t.name,
        // สถิติเซฟในข้อมูลการแข่งขันผูกกับทีม ไม่ได้ระบุว่านายทวารคนใดลงสนาม
        // ถ้าทีมมีหลาย GK จึงแสดงชื่อร่วมกันแทนการยกสถิติให้คนแรกโดยไม่มีหลักฐาน
        keeperName: keepers.map(keeper => keeper.name).filter(Boolean).join(' / ') || undefined,
        photoUrl: keepers.length === 1 ? keepers[0].photoUrl : undefined,
        saves: 0,
        cleanSheets: 0,
        logoUrl: t.logoUrl,
      };
    ...
    matches.forEach(m => {
      ...
      m.kicks?.forEach(k => {
        if (k.result !== KickResult.SAVED) return;
  ...
  /** แถบโลโก้มุมจอ — เอาเฉพาะรายที่มีโลโก้ ไม่งั้นได้ช่องว่างหรือรูปแตกคั่นอยู่ */
  const sponsorsWithLogo = useMemo(() => sponsors.filter(s => (s.logoUrl || '').trim() !== ''), [sponsors]);

  useEffect(() => { const clockTimer = setInterval(() => setCurrentTime(new Date()), 1000); return () => clearInterval(clockTimer); }, []);
  // ฉากใน OBS ที่ล็อกสไลด์ไว้ต้องไม่วนเอง ไม่งั้นฉาก "ผู้สนับสนุน" จะกลายเป็น
  // ตารางแข่งกลางออกอากาศโดยที่ Operator ไม่ได้สั่งอะไร
  useEffect(() => { if (!isAuthenticated || isSlidePaused || wallParams.slide !== null) return; const SLIDE_DURATIONS: Record<number, number> = { 1: 25000, 11: 45000, 12: 45000 }; /* ผังนักกีฬาวนทีมละ 8 วิ ถ้าให้เวลาสไลด์แค่ 15 วิเท่าอันอื่นจะเห็นแค่ทีมเดียวครึ่ง
              ...
              <div className="h-full flex flex-col animate-broadcast-reveal">
                <div className="flex items-center gap-4 mb-4 md:mb-6 mt-2">
                  <div className="bg-blue-600 p-2 rounded-lg shadow-[0_0_20px_rgba(37,99,235,0.5)]">
                    <Hand className="w-6 h-6 md:w-8 md:h-8 text-white" />
                  </div>
                  <div>
                    <h2 className="text-xl md:text-4xl font-black text-white uppercase tracking-tight">Golden Glove</h2>
                    <p className="text-[10px] md:text-sm text-blue-300 font-bold">ผู้รักษาประตูจากข้อมูลตำแหน่ง Lineup</p>
                  </div>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6 overflow-y-auto">
                  {topKeepers.length > 0 ? topKeepers.map((keeper, idx) => (
                    <div key={keeper.teamName}
                      className="bg-slate-900/80 border border-blue-500/20 rounded-2xl p-4 md:p-6 flex items-center justify-between gap-4 shadow-lg">
                      <div className="flex items-center gap-3 md:gap-4 min-w-0">
                        <div className={`w-8 h-8 md:w-12 md:h-12 rounded-full flex items-center justify-center font-black text-base md:text-xl shrink-0 ${idx === 0 ? 'bg-yellow-400 text-yellow-900' : 'bg-slate-700 text-white'}`}>
                          {idx + 1}
                        </div>
                        {(keeper.photoUrl || keeper.logoUrl) ? (
                          <img src={keeper.photoUrl || keeper.logoUrl}
                            className={`w-12 h-12 md:w-16 md:h-16 bg-white rounded-xl shrink-0 ${keeper.photoUrl ? 'object-cover' : 'object-contain p-1'}`} />
                        ) : (
                          <div className="w-12 h-12 md:w-16 md:h-16 bg-white/10 rounded-xl flex items-center justify-center text-xl md:text-2xl font-bold shrink-0">
                            {keeper.teamName.substring(0, 1)}
                          </div>
                        )}
                        <div className="min-w-0">
                          <h3 className="text-base md:text-xl font-bold truncate">{keeper.keeperName || 'ยังไม่ระบุผู้รักษาประตู'}</h3>
                          <p className="text-[10px] md:text-xs text-blue-400 font-bold truncate">
                            {keeper.teamName} · ผู้รักษาประตู
                          </p>
                        </div>
                      </div>
                      <div className="flex gap-4 text-center shrink-0">
                        <div>
                          <div className="text-xl md:text-3xl font-black text-white"><NumberCounter target={keeper.cleanSheets} /></div>
                          <div className="text-[8px] md:text-[10px] uppercase text-slate-500 font-bold">Clean Sheets</div>
                        </div>
                        <div>
                          <div className="text-xl md:text-3xl font-black text-blue-400"><NumberCounter target={keeper.saves} /></div>
                          <div className="text-[8px] md:text-[10px] uppercase text-slate-500 font-bold">PK Saves</div>
                        </div>
                      </div>
                    </div>
                  )) : (
                    <div className="col-span-1 md:col-span-2 flex items-center justify-center h-64 text-slate-500 text-xl md:text-2xl font-bold">
                      No Goalkeeper Data
                    </div>
                  )}
                </div>
                                ...
                                <div key={p.id ?? i} className="rounded-xl md:rounded-2xl bg-white/[0.06] border border-white/10 overflow-hidden animate-in fade-in duration-700">
                                    <div className="relative bg-slate-900" style={{ aspectRatio: '3 / 4' }}>
                                        ...
                                        <div className="absolute inset-x-0 bottom-0 p-1.5 md:p-2.5">
                                            ...
                                            <p className="text-[8px] md:text-xs" style={{ color: colors }}>{playerPositionLabel(p.position)}</p>
                                        </div>
                                    </div>
                                </div>
                            ))}
