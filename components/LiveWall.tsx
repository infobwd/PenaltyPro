...
import { Trophy, Clock, Calendar, MapPin, Activity, Award, Megaphone, Monitor, Maximize2, X, ChevronRight, Hand, Sparkles, Camera, Heart, User, QrCode, Settings, Plus, Trash2, Upload, Loader2, Save, Music, Play, Pause, SkipForward, Youtube, Volume2, VolumeX, Star, Zap, Keyboard, Info, Swords, Timer, Lock, Siren, CloudSun, Droplets } from 'lucide-react';
...
const WeatherWidget = () => {
    // Mock weather data
    const temp = 32;
    const condition = "ท้องฟ้าแจ่มใส";
    const humidity = 65;

    return (
        <div className="hidden xl:flex items-center gap-3 bg-white/5 backdrop-blur-md px-4 py-2 rounded-lg border border-white/10 shadow-lg">
            <CloudSun className="w-8 h-8 text-yellow-400" />
            <div className="flex flex-col items-start">
                <div className="text-xl font-bold text-white font-teko leading-none">{temp}°C</div>
                <div className="text-[12px] text-slate-300 font-kanit">{condition}</div>
            </div>
            <div className="h-6 w-[1px] bg-white/20 mx-1"></div>
            <div className="flex flex-col items-center">
                <div className="flex items-center gap-1 text-xs text-blue-300 font-teko">
...
const SettingsManagerModal: React.FC<{ 
    ...
    return (
        <div className="fixed inset-0 z-[6000] bg-black/80 flex items-center justify-center p-4 backdrop-blur-sm font-kanit" onClick={onClose}>
...
const LiveWall: React.FC<LiveWallProps> = ({ matches, teams, players, config, predictions, onClose, onRefresh }) => {
  ...
  // Alert Overlay State
  const [alertEvent, setAlertEvent] = useState<{ type: 'GOAL' | 'RESULT', match: Match } | null>(null);
  ...
  // --- GOAL ALERT LOGIC ---
  useEffect(() => {
      if (!isAuthenticated || prevMatchesRef.current.length === 0) {
      ...
      matches.forEach(newM => {
          ...
          if (oldM) {
              const scoreChanged = newM.scoreA > oldM.scoreA || newM.scoreB > oldM.scoreB;
              const justFinished = !oldM.winner && newM.winner;
              
              if (scoreChanged) {
                  setAlertEvent({ type: 'GOAL', match: newM });
                  setTimeout(() => setAlertEvent(null), 8000); // Show for 8s
              } else if (justFinished) {
                  setAlertEvent({ type: 'RESULT', match: newM });
                  setTimeout(() => setAlertEvent(null), 10000); // Show for 10s
              }
          }
      });
  ...
  useEffect(() => {
      ...
      if (alertEvent) return; // Pause rotation if Alert is showing
  ...
  }, [totalSlides, isAuthenticated, alertEvent]); 

  // Sub-rotation for paginated content (Standings, Highlights)
  useEffect(() => {
      if (!isAuthenticated) return;
      if (alertEvent) return; // Pause
  ...
  }, [currentSlide, standingsGroups.length, contestEntries.length, isAuthenticated, alertEvent]);
  ...
  const renderGoalAlert = () => {
      ...
      const { type, match } = alertEvent;
      const tA = resolveTeam(match.teamA);
      const tB = resolveTeam(match.teamB);

      return (
          <div className="fixed inset-0 z-[9999] flex flex-col items-center justify-center bg-black/80 backdrop-blur-xl animate-in zoom-in duration-300">
              <div className="w-full h-full absolute inset-0 overflow-hidden pointer-events-none">
                  <div className="absolute top-0 left-0 w-full h-2 bg-gradient-to-r from-red-600 via-yellow-500 to-red-600 animate-[shimmer_2s_infinite]"></div>
                  <div className="absolute bottom-0 right-0 w-full h-2 bg-gradient-to-l from-red-600 via-yellow-500 to-red-600 animate-[shimmer_2s_infinite]"></div>
              </div>
              
              <div className="text-center animate-in slide-in-from-bottom-20 duration-500">
                  <div className="inline-flex items-center gap-4 bg-red-600 px-8 py-3 rounded-full shadow-[0_0_50px_rgba(220,38,38,0.8)] mb-8 animate-bounce">
                      <Siren className="w-8 h-8 text-white animate-pulse" />
                      <h2 className="text-6xl font-black text-white font-teko uppercase tracking-widest">
                          {type === 'GOAL' ? 'GOAL ALERT' : 'FULL TIME'}
                      </h2>
                      <Siren className="w-8 h-8 text-white animate-pulse" />
                  </div>
                  ...
                  <div className="flex items-center gap-12 md:gap-24 mb-8">
                      <div className="flex flex-col items-center">
                          {tA.logoUrl ? <img src={tA.logoUrl} className="w-32 h-32 md:w-48 md:h-48 object-contain drop-shadow-2xl animate-[pulse_2s_infinite]" /> : <div className="text-6xl font-black text-slate-500">A</div>}
                          <h3 className="text-4xl font-bold text-white mt-4 font-kanit uppercase tracking-wider">{tA.name}</h3>
                      </div>
                      
                      <div className="flex flex-col items-center">
                          <div className="text-9xl md:text-[12rem] font-black text-white font-teko leading-none drop-shadow-[0_0_30px_rgba(255,255,255,0.5)]">
                              {match.scoreA} - {match.scoreB}
                          </div>
                          {type === 'GOAL' && <div className="text-3xl text-yellow-400 font-bold uppercase tracking-widest animate-pulse font-teko">Score Update</div>}
                      </div>

                      <div className="flex flex-col items-center">
                          {tB.logoUrl ? <img src={tB.logoUrl} className="w-32 h-32 md:w-48 md:h-48 object-contain drop-shadow-2xl animate-[pulse_2s_infinite]" /> : <div className="text-6xl font-black text-slate-500">B</div>}
                          <h3 className="text-4xl font-bold text-white mt-4 font-kanit uppercase tracking-wider">{tB.name}</h3>
                      </div>
                  </div>
              </div>
          </div>
      ...
      return (
          <div className="fixed inset-0 z-[6000] bg-slate-950 flex flex-col items-center justify-center font-kanit">
              ...
              <div className="relative z-10 w-full max-w-sm bg-white/10 backdrop-blur-xl p-8 rounded-2xl shadow-2xl border border-white/10 flex flex-col items-center animate-in zoom-in duration-300">
                  ...
                  <h2 className="text-4xl font-black text-white mb-2 tracking-wide font-teko">LOCKED</h2>
                  <p className="text-slate-400 text-sm mb-6 text-center font-kanit">กรุณากรอกรหัส PIN เพื่อเข้าสู่ Live Wall</p>
                  ...
                  <form onSubmit={handlePinSubmit} className="w-full flex flex-col gap-4">
                      ...
                      {pinError && (
                          <div className="text-red-400 text-xs font-bold text-center bg-red-900/20 p-2 rounded animate-pulse font-kanit">
                              รหัสผ่านไม่ถูกต้อง
                          </div>
                      )}
                      ...
                      <button 
                          type="submit" 
                          className="w-full py-3 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl font-bold shadow-lg transition transform active:scale-95 mt-2 font-teko text-2xl tracking-wide"
                      >
                          UNLOCK
                      </button>
                  </form>
                  
                  <button onClick={onClose} className="mt-6 text-slate-500 hover:text-white text-xs flex items-center gap-1 transition font-kanit">
  ...
  if (!hasInteracted) {
      return (
          <div className="fixed inset-0 z-[5000] bg-slate-950 flex flex-col items-center justify-center cursor-pointer" onClick={handleStartExperience}>
              ...
              <h1 className="text-6xl font-black text-white uppercase tracking-widest mb-2 font-kanit">{config.competitionName}</h1>
              <p className="text-slate-400 font-bold text-lg animate-bounce font-teko">CLICK ANYWHERE TO START</p>
              <p className="text-xs text-slate-600 mt-4 font-mono">Enabling Audio & Visuals</p>
          </div>
      );
  }
  ...
  return (
    <div className="fixed inset-0 z-[5000] bg-slate-950 text-white overflow-hidden flex flex-col font-kanit select-none cursor-none" style={{ fontFamily: "'Kanit', sans-serif" }}>
        ...
        {/* ANIMATED BACKGROUND - Darker and more premium */}
        <div className="absolute inset-0 z-0 overflow-hidden pointer-events-none transition-colors duration-1000">
            <div className={`absolute top-[-50%] left-[-50%] w-[200%] h-[200%] bg-[radial-gradient(circle_at_center,_var(--tw-gradient-stops))] ${currentSlide === 7 ? 'from-yellow-900/30 via-slate-950' : 'from-indigo-900/30 via-slate-950'} to-black animate-slow-spin opacity-60`}></div>
        ...
        <div className="h-24 bg-gradient-to-b from-slate-950 to-transparent flex items-center justify-between px-8 relative z-20 pt-4 group shrink-0">
            <div className="flex items-center gap-6">
                <div className="w-16 h-16 bg-white/5 backdrop-blur-md rounded-2xl p-2 shadow-[0_0_20px_rgba(99,102,241,0.2)] border border-white/10">
                    <img src={config.competitionLogo} className="w-full h-full object-contain drop-shadow-md" />
                </div>
                <div>
                    <h1 className="text-6xl font-black tracking-tighter text-transparent bg-clip-text bg-gradient-to-r from-white via-indigo-100 to-indigo-300 uppercase drop-shadow-sm font-kanit leading-none">
                    ...
                    <div className="flex items-center gap-3 mt-1">
                        ...
                        <span className="text-sm font-bold text-red-400 tracking-widest uppercase font-teko">Live Coverage</span>
                        {currentTrack ? (
                            ...
                            <div className="flex items-center gap-1 ml-4 text-xs text-slate-500 font-kanit"><VolumeX className="w-3 h-3"/> Audio Off</div>
                        )}
                    </div>
                </div>
            </div>
            ...
            <div className="flex items-center gap-8">
                {/* NEXT MATCH TEASER */}
                {nextMatch && (
                    <div className="hidden xl:flex flex-col items-end bg-black/40 backdrop-blur-md px-4 py-2 rounded-lg border border-white/5 shadow-lg">
                        <div className="text-[12px] text-indigo-300 font-bold uppercase tracking-wider mb-1 flex items-center gap-1 font-teko">
                            <Clock className="w-3 h-3"/> Next Match
                        </div>
                        <div className="text-xl font-bold text-white flex items-center gap-2 font-kanit tracking-wide">
                             <span>{resolveTeam(nextMatch.teamA).shortName || 'TBA'}</span>
                             <span className="text-slate-400 text-sm font-teko">vs</span>
                             <span>{resolveTeam(nextMatch.teamB).shortName || 'TBA'}</span>
                        </div>
                        <div className="text-[12px] text-slate-400 mt-0.5 font-teko">
                ...
                <div className="text-right">
                    <div className="text-6xl font-black font-teko leading-none tracking-widest text-white drop-shadow-[0_0_15px_rgba(255,255,255,0.6)]">
                        {currentTime.toLocaleTimeString('th-TH', { hour: '2-digit', minute: '2-digit' })}
                    </div>
                    <div className="text-sm text-slate-400 font-bold uppercase tracking-widest mt-0 font-kanit">
        ...
        >
            ...
            {currentSlide === 0 && (
                <div className="h-full flex flex-col animate-in fade-in zoom-in-95 duration-1000">
                    <div className="flex items-center gap-4 mb-8 pl-4 border-l-4 border-red-600">
                        <h2 className="text-7xl font-black text-white uppercase tracking-tight font-teko">Match Center</h2>
                    </div>
                    <div className="flex-1 flex items-center justify-center">
                        {upcomingMatches.length > 0 ? (
                                        ...
                                        <div key={m.id} className={`relative bg-slate-900/60 backdrop-blur-xl rounded-3xl border p-6 flex items-center justify-between transition-all duration-500 ${isLive ? 'border-red-500/50 shadow-[0_0_40px_rgba(220,38,38,0.15)] bg-gradient-to-r from-red-950/30 to-slate-900/60' : 'border-white/10'} ${idx === 0 ? 'scale-105 z-10' : 'scale-100 opacity-90'}`}>
                                            <div className="flex items-center gap-6 w-[40%]">
                                                <div className="w-20 h-20 bg-white/5 rounded-2xl p-2 shadow-inner border border-white/5 flex items-center justify-center shrink-0">
                                                    {tA.logoUrl ? <img src={tA.logoUrl} className="w-full h-full object-contain drop-shadow-md"/> : <div className="text-4xl font-black text-slate-600 font-teko">{tA.name.substring(0,1)}</div>}
                                                </div>
                                                <span className="text-4xl font-bold truncate text-white font-kanit uppercase tracking-wide">{tA.name}</span>
                                            </div>
                                            <div className="flex flex-col items-center w-[20%] relative">
                                                {isLive ? (
                                                    <div className="absolute -top-10 bg-red-600 text-white px-3 py-0.5 rounded text-xl font-bold uppercase tracking-wider animate-pulse font-teko">Live</div>
                                                ) : (
                                                    <div className="absolute -top-10 bg-indigo-500/20 text-indigo-300 border border-indigo-500/30 px-3 py-0.5 rounded text-xl font-bold uppercase tracking-wider font-teko">
                                                        {new Date(m.scheduledTime || m.date).toLocaleTimeString('th-TH', {hour:'2-digit', minute:'2-digit'})}
                                                    </div>
                                                )}
                                                <div className="flex items-center gap-4 text-8xl font-black font-teko tracking-tighter text-white drop-shadow-[0_0_15px_rgba(255,255,255,0.3)] leading-none -mt-2">
                                                    <span>{m.scoreA}</span>
                                                    <span className="text-slate-600 text-6xl">:</span>
                                                    <span>{m.scoreB}</span>
                                                </div>
                                                <div className="text-indigo-400 font-bold text-xl tracking-widest mt-2 uppercase font-kanit">{m.roundLabel?.split(':')[0]}</div>
                                            </div>
                                            <div className="flex items-center gap-6 w-[40%] justify-end">
                                                <span className="text-4xl font-bold truncate text-right text-white font-kanit uppercase tracking-wide">{tB.name}</span>
                                                <div className="w-20 h-20 bg-white/5 rounded-2xl p-2 shadow-inner border border-white/5 flex items-center justify-center shrink-0">
                                                    {tB.logoUrl ? <img src={tB.logoUrl} className="w-full h-full object-contain drop-shadow-md"/> : <div className="text-4xl font-black text-slate-600 font-teko">{tB.name.substring(0,1)}</div>}
                                                </div>
                                            </div>
                                        </div>
                                    );
                            ...
                            <div className="text-center opacity-40 flex flex-col items-center">
                                ...
                                <h3 className="text-6xl font-black tracking-widest font-teko">NO MATCHES NOW</h3>
                            </div>
                        )}
                    </div>
                </div>
            ...
            {currentSlide === 1 && (
                <div className="h-full flex flex-col animate-in fade-in slide-in-from-right-10 duration-1000">
                    <div className="flex items-center justify-between mb-8 pl-4 border-l-4 border-indigo-500">
                        <div className="flex items-center gap-4">
                            <h2 className="text-7xl font-black text-white uppercase tracking-tight font-teko">Current Standings</h2>
                        </div>
                    ...
                    <div className="flex-1 relative">
                                ...
                                {standingsGroups[standingsPage]?.map(group => (
                                    <div key={group.name} className="bg-slate-900/80 border border-white/10 rounded-2xl overflow-hidden backdrop-blur-md shadow-2xl">
                                        <div className="bg-gradient-to-r from-indigo-900/50 to-slate-900/50 px-6 py-4 border-b border-white/5 flex justify-between items-center">
                                            <h3 className="font-black text-4xl text-white flex items-center gap-2 font-kanit"><span className="text-indigo-400">กลุ่ม</span> {group.name}</h3>
                                            <div className="text-sm font-bold text-slate-500 bg-white/5 px-2 py-1 rounded font-kanit">Top 2 Qualify</div>
                                        </div>
                                        <table className="w-full text-lg">
                                            <thead className="bg-white/5 text-slate-400 text-lg uppercase tracking-wider font-bold font-teko">
                                                ...
                                                {group.teams.map((team, idx) => (
                                                    <tr key={team.teamId} className={`transition-colors ${idx < 2 ? "bg-green-500/5" : ""}`}>
                                                        <td className="p-3 pl-6 font-bold flex items-center gap-4">
                                                            <div className={`w-8 h-8 rounded flex items-center justify-center text-xl font-black font-teko ${idx < 2 ? 'bg-green-500 text-black' : 'bg-slate-700 text-slate-400'}`}>{idx+1}</div>
                                                            {team.logoUrl && <img src={team.logoUrl} className="w-8 h-8 object-contain" />}
                                                            <span className="truncate max-w-[220px] text-2xl font-kanit tracking-wide">{team.teamName}</span>
                                                        </td>
                                                        <td className="p-3 text-center text-slate-400 font-teko text-2xl">{team.played}</td>
                                                        <td className="p-3 text-center text-slate-400 font-teko text-2xl">{team.goalsFor - team.goalsAgainst}</td>
                                                        <td className="p-3 text-center font-black text-yellow-400 text-4xl font-teko bg-white/5">{team.points}</td>
                                                    </tr>
                                                ))}
                                            </tbody>
                                        </table>
                        ...
                        ) : <div className="flex items-center justify-center h-full text-slate-500 text-2xl font-bold font-kanit">Waiting for standings...</div>}
                    </div>
                </div>
            )}
            ...
            {currentSlide === 2 && (
                <div className="h-full flex flex-col animate-in fade-in slide-in-from-bottom-10 duration-1000">
                    <div className="flex items-center gap-4 mb-8 pl-4 border-l-4 border-green-600">
                        <h2 className="text-7xl font-black text-white uppercase tracking-tight font-teko">Match Results</h2>
                    </div>
                    ...
                    {recentResults.length > 0 ? (
                            ...
                            {recentResults.map((m) => {
                                ...
                                return (
                                    <div key={m.id} className="relative bg-slate-900/80 backdrop-blur-md rounded-2xl border border-white/10 p-0 overflow-hidden shadow-2xl transition-all duration-500">
                                        <div className="bg-white/5 px-4 py-2 flex justify-between items-center text-sm font-bold text-slate-400 font-kanit">
                                            <span className="font-mono">{new Date(m.date).toLocaleDateString('th-TH', {day:'numeric', month:'short'})}</span>
                                        ...
                                        <div className="p-6 flex items-center justify-between">
                                            ...
                                            <div className={`flex flex-col items-center gap-3 flex-1 ${winnerA ? 'opacity-100 scale-105' : 'opacity-60 grayscale-[0.5]'}`}>
                                                <div className={`w-20 h-20 rounded-2xl p-2 flex items-center justify-center bg-gradient-to-br ${winnerA ? 'from-green-500/20 to-emerald-900/40 border border-green-500/50 shadow-[0_0_20px_rgba(34,197,94,0.2)]' : 'from-slate-800 to-slate-900 border border-white/5'}`}>
                                                    {tA.logoUrl ? <img src={tA.logoUrl} className="w-full h-full object-contain drop-shadow-md"/> : <div className="text-4xl font-black text-slate-500 font-teko">{tA.name.substring(0,1)}</div>}
                                                </div>
                                                <span className={`text-xl font-bold text-center leading-tight font-kanit uppercase ${winnerA ? 'text-white' : 'text-slate-400'}`}>{tA.name}</span>
                                            </div>

                                            {/* Score */}
                                            <div className="flex flex-col items-center px-4">
                                                <div className="text-8xl font-black font-teko text-white tracking-tighter drop-shadow-[0_4px_10px_rgba(0,0,0,0.5)] leading-none">
                                                    {m.scoreA}-{m.scoreB}
                                                </div>
                                                <div className="mt-2 bg-white/10 px-2 py-0.5 rounded text-xs font-bold text-yellow-400 border border-yellow-500/30 uppercase tracking-widest font-teko">
                                                    Full Time
                                                </div>
                                            </div>
                                            ...
                                            <div className={`flex flex-col items-center gap-3 flex-1 ${winnerB ? 'opacity-100 scale-105' : 'opacity-60 grayscale-[0.5]'}`}>
                                                <div className={`w-20 h-20 rounded-2xl p-2 flex items-center justify-center bg-gradient-to-br ${winnerB ? 'from-green-500/20 to-emerald-900/40 border border-green-500/50 shadow-[0_0_20px_rgba(34,197,94,0.2)]' : 'from-slate-800 to-slate-900 border border-white/5'}`}>
                                                    {tB.logoUrl ? <img src={tB.logoUrl} className="w-full h-full object-contain drop-shadow-md"/> : <div className="text-4xl font-black text-slate-500 font-teko">{tB.name.substring(0,1)}</div>}
                                                </div>
                                                <span className={`text-xl font-bold text-center leading-tight font-kanit uppercase ${winnerB ? 'text-white' : 'text-slate-400'}`}>{tB.name}</span>
                                            </div>
                                        </div>
                                    </div>
                                );
                        ...
                        <div className="flex-1 flex items-center justify-center text-slate-500 text-2xl font-bold font-kanit">No finished matches yet</div>
                    )}
                </div>
            )}
            ...
            {currentSlide === 3 && (
                <div className="h-full flex flex-col animate-in zoom-in-95 duration-1000">
                    <div className="text-center mb-8">
                        <h2 className="text-8xl font-black text-yellow-400 uppercase tracking-tighter drop-shadow-lg font-teko">Golden Boot</h2>
                        <p className="text-slate-400 font-bold uppercase tracking-widest mt-1 text-2xl font-kanit">ดาวซัลโวสูงสุด</p>
                    </div>
                    {topScorers.length > 0 ? (
                            ...
                            {topScorers[1] && (
                                <div className="flex flex-col items-center w-64 animate-in slide-in-from-bottom-20 duration-1000 delay-100">
                                    ...
                                    <div className="bg-slate-800 w-full p-4 rounded-t-2xl text-center border-t-4 border-slate-500 relative">
                                        <div className="absolute -top-5 left-1/2 -translate-x-1/2 bg-slate-600 text-white w-10 h-10 rounded-full flex items-center justify-center font-black border-4 border-slate-800 font-teko text-2xl">2</div>
                                        <h3 className="font-bold text-2xl truncate mt-2 font-kanit tracking-wide">{topScorers[1].name}</h3>
                                        <p className="text-sm text-slate-400 uppercase mb-2 font-kanit">{topScorers[1].team}</p>
                                        <div className="text-7xl font-black text-white font-teko leading-none">{topScorers[1].goals}</div>
                                        <div className="text-xs uppercase font-bold text-slate-500 font-teko tracking-widest">Goals</div>
                                    </div>
                            ...
                            {topScorers[0] && (
                                <div className="flex flex-col items-center w-72 z-10 animate-in slide-in-from-bottom-32 duration-1000">
                                    ...
                                    <div className="bg-gradient-to-b from-yellow-600 to-yellow-700 w-full p-6 rounded-t-3xl text-center border-t-4 border-yellow-300 relative shadow-2xl">
                                        <div className="absolute -top-6 left-1/2 -translate-x-1/2 bg-yellow-400 text-yellow-900 w-14 h-14 rounded-full flex items-center justify-center font-black text-3xl border-4 border-yellow-600 font-teko">1</div>
                                        <h3 className="font-black text-4xl truncate mt-2 text-white font-kanit tracking-wide">{topScorers[0].name}</h3>
                                        <p className="text-lg text-yellow-200 uppercase mb-3 font-bold font-kanit">{topScorers[0].team}</p>
                                        <div className="text-9xl font-black text-white drop-shadow-md font-teko leading-none">{topScorers[0].goals}</div>
                                        <div className="text-sm uppercase font-bold text-yellow-200/80 tracking-widest font-teko">Goals Scored</div>
                                    </div>
                            ...
                            {topScorers[2] && (
                                <div className="flex flex-col items-center w-64 animate-in slide-in-from-bottom-20 duration-1000 delay-200">
                                    ...
                                    <div className="bg-orange-900 w-full p-4 rounded-t-2xl text-center border-t-4 border-orange-600 relative">
                                        <div className="absolute -top-5 left-1/2 -translate-x-1/2 bg-orange-600 text-white w-10 h-10 rounded-full flex items-center justify-center font-black border-4 border-orange-900 font-teko text-2xl">3</div>
                                        <h3 className="font-bold text-2xl truncate mt-2 text-orange-100 font-kanit tracking-wide">{topScorers[2].name}</h3>
                                        <p className="text-sm text-orange-400 uppercase mb-2 font-kanit">{topScorers[2].team}</p>
                                        <div className="text-7xl font-black text-white font-teko leading-none">{topScorers[2].goals}</div>
                                        <div className="text-xs uppercase font-bold text-orange-400 font-teko tracking-widest">Goals</div>
                                    </div>
                        ...
                        <div className="flex-1 flex items-center justify-center text-slate-500 text-2xl font-bold font-kanit">No Goalscorers Yet</div>
                    )}
                </div>
            )}
            ...
            {currentSlide === 4 && (
                <div className="h-full flex flex-col animate-in fade-in slide-in-from-right-10 duration-1000">
                    <div className="flex items-center gap-4 mb-8 pl-4 border-l-4 border-blue-600">
                        <h2 className="text-7xl font-black text-white uppercase tracking-tight font-teko">Golden Glove</h2>
                    </div>
                    <div className="grid grid-cols-2 gap-6">
                        {topKeepers.length > 0 ? topKeepers.map((k, idx) => (
                            <div key={idx} className="bg-slate-900/80 border border-blue-500/20 rounded-2xl p-6 flex items-center justify-between shadow-lg">
                                <div className="flex items-center gap-4">
                                    <div className={`w-12 h-12 rounded-full flex items-center justify-center font-black text-2xl font-teko ${idx === 0 ? 'bg-yellow-400 text-yellow-900' : 'bg-slate-700 text-white'}`}>{idx+1}</div>
                                    <div>
                                        <h3 className="text-4xl font-bold font-kanit tracking-wide">{k.teamName}</h3>
                                        <p className="text-sm text-blue-400 uppercase font-bold font-teko tracking-wider">Goalkeeper</p>
                                    </div>
                                </div>
                                <div className="flex gap-4 text-center">
                                    <div>
                                        <div className="text-6xl font-black text-white font-teko leading-none">{k.cleanSheets}</div>
                                        <div className="text-xs uppercase text-slate-500 font-bold font-teko tracking-widest">Clean Sheets</div>
                                    </div>
                                    <div>
                                        <div className="text-6xl font-black text-blue-400 font-teko leading-none">{k.saves}</div>
                                        <div className="text-xs uppercase text-slate-500 font-bold font-teko tracking-widest">PK Saves</div>
                                    </div>
                                </div>
                            </div>
                        )) : <div className="col-span-2 flex items-center justify-center h-64 text-slate-500 text-2xl font-bold font-kanit">No Goalkeeper Data</div>}
                    </div>
                </div>
            )}
            ...
            {currentSlide === 5 && (
                <div className="h-full flex flex-col animate-in zoom-in-95 duration-1000 relative">
                    <div className="text-center mb-8">
                        <h2 className="text-8xl font-black text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-pink-400 uppercase tracking-tighter drop-shadow-lg font-teko">Fan Zone Leaderboard</h2>
                    </div>
                    <div className="flex-1 flex flex-col gap-4 max-w-4xl mx-auto w-full">
                        {fanRankings.length > 0 ? fanRankings.map((fan, idx) => (
                            <div key={idx} className={`bg-white/5 rounded-2xl p-4 flex items-center justify-between border border-white/10 ${idx === 0 ? 'bg-gradient-to-r from-yellow-500/20 to-purple-500/20 border-yellow-500/50' : ''}`}>
                                <div className="flex items-center gap-4">
                                    <div className={`w-12 h-12 flex items-center justify-center font-black text-2xl font-teko rounded-full ${idx < 3 ? 'bg-white text-black' : 'bg-slate-700 text-slate-400'}`}>
                                        {idx + 1}
                                    </div>
                                    ...
                                    <span className="text-4xl font-bold font-kanit tracking-wide">{fan.name}</span>
                                </div>
                                <div className="text-right">
                                    <div className="text-6xl font-black text-purple-400 font-teko leading-none">{fan.points} <span className="text-lg text-slate-500 font-bold font-teko">PTS</span></div>
                                    <div className="text-sm text-slate-400 font-kanit">Correct: {fan.correct}</div>
                                </div>
                            </div>
                        )) : <div className="flex items-center justify-center h-full text-slate-500 text-2xl font-bold font-kanit">No Predictions Yet</div>}
                    </div>
                </div>
            )}
                            ...
                            <div className="relative w-full aspect-video rounded-3xl overflow-hidden shadow-2xl border-4 border-white/20 group">
                                ...
                                <div className="absolute bottom-0 inset-x-0 bg-gradient-to-t from-black/80 to-transparent p-8">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            <h3 className="text-3xl font-bold text-white font-kanit">{contestEntries[highlightIndex].caption}</h3>
                                            <p className="text-slate-300 font-medium font-kanit">โดย {contestEntries[highlightIndex].userDisplayName}</p>
                                        </div>
                                        <div className="ml-auto flex items-center gap-2 bg-pink-600 px-4 py-2 rounded-full">
                                            <Heart className="w-6 h-6 fill-white" />
                                            <span className="font-bold text-2xl font-teko">{contestEntries[highlightIndex].likeCount}</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <h2 className="text-5xl font-black text-white uppercase tracking-widest mt-8 flex items-center gap-3 font-teko">
                                <Camera className="w-10 h-10 text-pink-500" /> Photo Contest Highlights
                            </h2>
                        </div>
                    </div>
                ) : <div className="flex items-center justify-center h-full text-slate-500 text-2xl font-bold font-kanit">No Photos Yet</div>
            )}
                    ...
                    <div className="relative z-10 w-full max-w-7xl px-8 flex flex-col items-center justify-center h-full">
                        ...
                        <div className="text-center mb-16 animate-in slide-in-from-top-10 duration-1000">
                            <div className="inline-flex items-center gap-3 bg-white/10 backdrop-blur-md px-6 py-2 rounded-full border border-white/20 shadow-[0_0_30px_rgba(255,255,255,0.1)] mb-4">
                                ...
                                <span className="text-sm font-bold text-white tracking-widest uppercase font-teko">Premium Partners</span>
                                <Star className="w-5 h-5 text-yellow-400 fill-yellow-400 animate-pulse" />
                            </div>
                            <h2 className="text-8xl md:text-[10rem] font-black text-transparent bg-clip-text bg-gradient-to-b from-white via-indigo-100 to-slate-500 uppercase tracking-tighter drop-shadow-2xl font-teko">
                                Official Sponsors
                            </h2>
                        </div>
                        ...
                        {sponsors.length > 0 ? (
                                ...
                                {sponsors.map((s, idx) => (
                                    ...
                                    >
                                        ...
                                        <div className="absolute bottom-4 left-0 right-0 text-center">
                                            <span className="text-xl font-bold text-white bg-black/50 px-4 py-1 rounded-full backdrop-blur-sm font-kanit">
                                                {s.name}
                                            </span>
                                        </div>
                                    </div>
                            ...
                            <div className="flex flex-col items-center animate-pulse">
                                ...
                                <div className="text-4xl font-bold text-slate-500 font-teko">Become a Partner</div>
                            </div>
                        )}
                    </div>
                </div>
                    ...
                    {nextMatch ? (
                        <>
                            ...
                            <div className="relative z-10 flex-1 flex flex-col justify-center items-center w-full max-w-7xl mx-auto px-4">
                                ...
                                {/* Header */}
                                <div className="text-center mb-8">
                                    <span className="bg-white/10 backdrop-blur-md border border-white/20 px-4 py-1 rounded-full text-indigo-300 font-bold tracking-widest text-lg uppercase mb-2 inline-block font-teko">
                                        Coming Up Next
                                    </span>
                                    <h2 className="text-7xl font-black text-white drop-shadow-[0_0_10px_rgba(255,255,255,0.5)] font-kanit uppercase">
                                ...
                                <div className="flex items-center justify-center w-full gap-8 md:gap-20">
                                    ...
                                    <div className="flex flex-col items-center w-1/3 animate-in slide-in-from-left-20 duration-1000">
                                        <div className="w-48 h-48 md:w-64 md:h-64 bg-white/5 rounded-full p-4 border-4 border-blue-500/50 shadow-[0_0_60px_rgba(59,130,246,0.3)] backdrop-blur-sm flex items-center justify-center mb-6 relative group">
                                            ...
                                            {resolveTeam(nextMatch.teamA).logoUrl ? (
                                                ...
                                                <div className="text-9xl font-black text-white/20 font-teko">A</div>
                                            )}
                                        </div>
                                        <h3 className="text-6xl md:text-8xl font-black text-white text-center leading-tight drop-shadow-lg uppercase font-kanit">
                                    ...
                                    <div className="flex flex-col items-center justify-center relative z-20">
                                        <div className="text-9xl font-black text-transparent bg-clip-text bg-gradient-to-b from-yellow-300 to-yellow-600 italic tracking-tighter drop-shadow-[0_5px_5px_rgba(0,0,0,0.5)] transform scale-150 mb-8 font-teko">
                                            VS
                                        </div>
                                        ...
                                        {countdown && (
                                            <div className="bg-black/50 backdrop-blur-md border border-white/10 px-6 py-3 rounded-2xl flex flex-col items-center gap-1 shadow-2xl">
                                                <span className="text-sm text-slate-400 font-bold uppercase tracking-widest font-teko">Kick Off In</span>
                                                <div className="text-8xl font-mono font-bold text-white tracking-widest tabular-nums text-shadow-glow font-teko leading-none">
                                                    {countdown}
                                                </div>
                                            </div>
                                        )}
                                    ...
                                    <div className="flex flex-col items-center w-1/3 animate-in slide-in-from-right-20 duration-1000">
                                        <div className="w-48 h-48 md:w-64 md:h-64 bg-white/5 rounded-full p-4 border-4 border-red-500/50 shadow-[0_0_60px_rgba(239,68,68,0.3)] backdrop-blur-sm flex items-center justify-center mb-6 relative group">
                                            ...
                                            {resolveTeam(nextMatch.teamB).logoUrl ? (
                                                ...
                                                <div className="text-9xl font-black text-white/20 font-teko">B</div>
                                            )}
                                        </div>
                                        <h3 className="text-6xl md:text-8xl font-black text-white text-center leading-tight drop-shadow-lg uppercase font-kanit">
                                ...
                                {/* Venue Info */}
                                <div className="mt-12 flex items-center gap-6 text-slate-300 text-4xl font-bold bg-black/30 px-12 py-4 rounded-full border border-white/5 backdrop-blur-sm font-kanit tracking-wide">
                                    <span className="flex items-center gap-2"><MapPin className="w-8 h-8 text-red-500"/> {nextMatch.venue || 'Main Stadium'}</span>
                                    <span className="w-2 h-2 bg-slate-500 rounded-full"></span>
                                    <span className="flex items-center gap-2"><Clock className="w-8 h-8 text-indigo-500"/> {new Date(nextMatch.scheduledTime || nextMatch.date).toLocaleTimeString('th-TH', {hour:'2-digit', minute:'2-digit'})}</span>
                                </div>
                            </div>
                        </>
                    ) : (
                        <div className="flex-1 flex flex-col items-center justify-center text-slate-500">
                            ...
                            <h2 className="text-6xl font-black uppercase tracking-widest opacity-50 font-teko">Tournament Continues</h2>
                            <p className="text-2xl mt-2 font-teko">Stay Tuned for More Action</p>
                        </div>
                    )}
                </div>
            )}
        ...
        <div className="h-24 bg-white/95 backdrop-blur-xl text-slate-900 flex items-center relative z-20 shadow-[0_-10px_50px_rgba(0,0,0,0.5)] border-t border-slate-200 shrink-0">
            <div className="bg-red-600 h-full px-12 flex items-center justify-center shrink-0 skew-x-[-10deg] -ml-6 shadow-xl z-20 relative overflow-hidden">
                ...
                <span className="text-white font-black uppercase tracking-widest flex items-center gap-2 skew-x-[10deg] text-4xl relative z-10 drop-shadow-md font-teko">
            ...
            <div className="flex-1 overflow-hidden relative h-full flex items-center z-10">
                <div className="absolute whitespace-nowrap animate-marquee px-4 text-4xl font-black text-slate-800 uppercase tracking-wide flex items-center font-kanit">
                    {announcements.length > 0 ? announcements.map((a, i) => (
                        <React.Fragment key={i}>
                            <span className="mx-12 font-kanit text-3xl">{a}</span>
            ...
            <div className="h-full bg-gradient-to-l from-slate-100 to-white flex items-center px-8 gap-6 z-20 border-l border-slate-200 min-w-[300px] justify-end relative overflow-hidden">
                <span className="text-[14px] text-slate-400 font-bold uppercase tracking-widest absolute top-2 right-4 font-teko">Official Partners</span>
                {sponsors.length > 0 ? (
