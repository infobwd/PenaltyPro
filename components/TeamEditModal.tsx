...
import { X, Save, Loader2, User, Plus, Trash2, Camera, FileText, Image as ImageIcon, CreditCard, ExternalLink, Shield, MapPin, Phone, Check, AlertTriangle, Clapperboard, PlayCircle } from 'lucide-react';
import { fileToBase64, setLineupMedia } from '../services/sheetService';
...
const TeamEditModal: React.FC<TeamEditModalProps> = ({ isOpen, onClose, team, currentPlayers, onSave, isAdmin = false }) => {
  ...
  const [activeTab, setActiveTab] = useState<'info' | 'players' | 'docs' | 'media'>('info');
  const [isSaving, setIsSaving] = useState(false);

  /**
   * สื่อสำหรับผังตัวนักกีฬา — เก็บแยกจาก formData/players โดยตั้งใจ
   *
   * บันทึกด้วย setLineupMedia ซึ่งไม่แตะรายชื่อและไม่ถอนสถานะอนุมัติทีม
   * ถ้ารวมเข้าปุ่ม "บันทึกข้อมูล" ด้านล่าง การแก้แค่ลิงก์คลิปจะส่งรายชื่อทั้งชุด
   * กลับไปด้วย ซึ่งพอทีมนั้นอนุมัติแล้วจะถูกถอนอนุมัติทันที (ดู save_team)
   */
  ...
  useEffect(() => {
    if (isOpen) {
        ...
        setPlayerVideos(Object.fromEntries(
            currentPlayers.map(p => [p.id, p.introVideoUrl || ''])));
  ...
  /**
   * บันทึกเฉพาะสื่อสำหรับผังตัวนักกีฬา
   *
   * ไม่เรียก onSave ของหน้าแม่ — ทางนั้นวิ่งเข้า saveTeam ซึ่งส่งรายชื่อทั้งชุด
   * กลับไปด้วย และจะถอนสถานะอนุมัติของทีมที่ผ่านการตรวจไปแล้ว
   */
    ...
    <div className="fixed inset-0 z-[1400] bg-black/60 backdrop-blur-sm modal-sheet flex items-end md:items-center justify-center p-0 md:p-4 overflow-y-auto" onClick={onClose}>
      <div className="bg-white w-full max-w-3xl rounded-2xl shadow-2xl overflow-hidden animate-in zoom-in duration-200 flex flex-col max-h-[90vh]" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex border-b border-slate-200 bg-slate-50 shrink-0">
            ...
            {/* งานเตรียมออกอากาศเป็นของเจ้าภาพ ไม่ใช่ของโรงเรียนที่มากรอกรายชื่อ */}
            {isAdmin && (
                <button onClick={() => setActiveTab('media')} className={`flex-1 py-3 text-sm font-bold transition ${activeTab === 'media' ? 'bg-white text-indigo-600 border-b-2 border-indigo-600' : 'text-slate-500 hover:bg-slate-100'}`}>ออกอากาศ</button>
            )}
        </div>
        ...
        <div className="p-6 overflow-y-auto flex-1 bg-white">
            ...
            {activeTab === 'media' && isAdmin && (
                <div className="space-y-6">
                    ...
                    <button onClick={handleSaveMedia} disabled={isSavingMedia}
                        ...
                        {isSavingMedia ? <Loader2 className="w-5 h-5 animate-spin" /> : <><Save className="w-5 h-5" /> บันทึกคลิปและคำโปรย</>}
                    </button>
                </div>
            )}
        </div>
